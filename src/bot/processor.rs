...
use super::{
    currency::{convert_currency, fetch_currency_conversion},
...
async fn retrieve_debts_converted(chat_id: &str) -> Result<Vec<Debt>, ProcessError> {
    ...
    for balances_currency in &mut balances {
        ...
        for balance in balances_currency {
            ...
            if should_convert {
                amount = convert_currency(amount, &currency, &default_currency, conversion_rate);
            }
...
async fn retrieve_spending_data_converted(chat_id: &str) -> Result<SpendingData, ProcessError> {
    ...
    for spending_currency in &mut spendings {
        ...
        for spending in spending_currency {
            ...
            if should_convert {
                spending_amount = convert_currency(
                    spending_amount,
                    &currency,
                    &default_currency,
                    conversion_rate,
                ...
                paid_amount =
                    convert_currency(paid_amount, &currency, &default_currency, conversion_rate);
            }
        ...
        for balance in balances_currency {
            if balance.balance != 0 {
                let user = user_spendings
                    .iter()
                    .position(|spending| spending.username == balance.username);
                let converted_balance = convert_currency(
                    balance.balance,
                    &currency,
                    &default_currency,
                    conversion_rate,
